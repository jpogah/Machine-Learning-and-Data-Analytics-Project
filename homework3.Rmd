...
$\sum_{i=1}^{N}{\bf _j<x_i>(P(y= +1\mid {\bf x_i, \theta}) -\mathbb{I}( y_i=+1)) }$
...
-The code for this question is shown in listing 3.3 . After trying different combination of number of iteration and step size , the best accuracy for my model  on the test data was  ~97% with the step size of 0.01 and maximum number of iteration of 300. The glm package recorded highest accuracy on test data of ~96% slighly below the performance of my model.
...
##Repeat (4), but replace the 70%-30% train-test split with each of the following splits:5%-95%, 10%-90%, 95%-5%. Graph the accuracy over the training set and over the testing set as a function of the size of the train set. Remember to average the accuracy over 10 random divisions of the data into train and test sets of the above sizes so the graphs will be less noisy
 ...
 - The code for this question is shown in listing 5.1,we see the train accuracy decreasing with increasing data size if we ignore the noise and also the test accuracy increasing with increasing data up to using 90% of the data for training(ignoring the noise). When 95% of the data is used for training we see a sharp decrease in the test accuracy.
```{r}
...
ggplot(result,aes(x=training_data)) + geom_line(aes(y=accuracy_train_data,color="train_accuracy")) + geom_line(aes(y=accuracy_test_data,color="test_accuracy")) +
  geom_point(aes(y=accuracy_train_data,color="train_accuracy")) + geom_point(aes(y=accuracy_test_data,color="test_accuracy")) + 
                   ylab("accuracy") +xlab("% training data")
...
##Repeat (5) but instead of graphing the train and test accuracy, graph the logistic regression loss function (negative log likelihood) over the train set and over the test set as a function of the train set size.
 ...
 - The code is shown in listing 6.1 , we can see the loss decreasing on the test data as the size of training data increases but the loss increases for the train data this is consistent with the result obtained in the graph for question 5.
...
train_loss_glm = compute_loss_function(train_data_matrix,
                                          weight,train_target)
test_data_matrix = get.feature.matrix(test_data,features)
test_target = label_array(test_data,"Class")
test_loss_glm = compute_loss_function(test_data_matrix, 
                                         weight,test_target)
  
  loss.on.train.data[num] = mean(train_loss_glm)/nrow(train_data)
  loss.test.data[num] = mean(test_loss_glm)/nrow(test_data)
}
